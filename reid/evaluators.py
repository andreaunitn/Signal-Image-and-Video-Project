...
def extract_features(model, data_loader, print_freq = 1, metric = None):
    ...
    for i, (imgs, fnames, pids, _) in enumerate(data_loader):
        ...
        if (i + 1) % print_freq == 0:
            print('Extract Features: [{}/{}]\t Time {:.3f} ({:.3f})\t Data {:.3f} ({:.3f})\t'.format(i + 1, len(data_loader), batch_time.val, batch_time.avg, data_time.val, data_time.avg))
...
def evaluate_all(distmat, query = None, gallery = None, query_ids = None, gallery_ids = None, query_cams = None, gallery_cams = None, cmc_topk = (1, 5, 10)):
    ...
    for k in cmc_topk:
        print('  top-{:<4}{:12.1%}{:12.1%}{:12.1%}'.format(k, cmc_scores['allshots'][k - 1], cmc_scores['cuhk03'][k - 1], cmc_scores['market1501'][k - 1]))

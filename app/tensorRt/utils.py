...
class BaseEngine(object):
    ...
    def detect_video(self, video_path, conf=0.5, end2end=False):
        ...
        while True:
            ...
            fps = (fps + (1. / (time.time() + 0.000001 - t1))) / 2
    ...
    # CUSTOM
    def customProcessVideo(self, ret, frame, width, height, end2end = True, conf=0.5):
        ...
        if dets is not None:
            ...
            frame, resulting_boxes = customVis(frame, final_boxes, final_scores, final_cls_inds, conf=conf, class_names=self.class_names)

        return frame, resulting_boxes
...
# CUSTOM
def customVis(img, boxes, scores, cls_ids, conf=0.5, class_names=None):
    ...
    for i in range(len(boxes)):
        ...
        cv2.putText(img, text, (x0, y0 + txt_size[1]), font, 0.4, txt_color, thickness=1)
        if(cls_id == 0):
            resulting_boxes.append(tuple((x0, y0, x1, y1)))

    return img, resulting_boxes